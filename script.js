...
async function setupCreateMap() {
  ...
  try {
    ...
    createMap.addListener("click", async (event) => {
      try {
        await handleCreateMapClick(event.latLng);
      } catch (error) {
        debugError("Map click location selection failed", error);
        showToast("Could not use that point on the map.");
      }
    });
...
async function handleCreateMapClick(latLng) {
  if (!latLng) {
    return;
  }
  ...
  debugLog("Create map clicked", { lat, lng });
  ...
  if (geocoder) {
    ...
    if (result) {
      selectedGooglePlace.formatted_address = result.formatted_address || selectedGooglePlace.formatted_address;
      selectedGooglePlace.name = result.address_components?.[0]?.long_name || "";
